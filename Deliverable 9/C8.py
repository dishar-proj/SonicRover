...
def turn_right():
    ...
    time.sleep(1)  # Adjust this time to achieve a 90-degree turn
    control_motor(0, 0, 0, 0)  # Stop motors
    time.sleep(0.1)
...
def move_square():
    for _ in range(4):
        move_distance(3.4)  # Move forward 3 feet
        time.sleep(1)
...
def single_sound_behavior():
    ...
    while not stopped:
        ...
        if adc_value == 0b000:
            ...
            control_motor(1, 0, 80, 0)  # Left turn (normal speed)
        
        # State 1: Turn left slow
        elif adc_value == 0b001:
            print("ADC: {} - Turning Left (Slower)".format(bin(adc_value)))
            control_motor(1, 0, 72.5, 0)  # Slower left turn
        
        # State 2: Turn left slowest
        elif adc_value == 0b011:
            print("ADC: {} - Slowing Down Even More but Still Turning Left".format(bin(adc_value)))
            control_motor(1, 0, 60, 0)  # Even slower left turn
        
        # State 3: Stop, Reread, Forward
